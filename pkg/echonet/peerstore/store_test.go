...
func TestPeerStore_AddAndGet(t *testing.T) {
	store := New()
	peer := &types.PeerInfo{UserID: "peer1", Multiaddress: "/ip4/127.0.0.1/tcp/4001"}
...
func TestPeerStore_AddInvalid(t *testing.T) {
	...
	if err := store.Add(&types.PeerInfo{UserID: ""}); err == nil {
...
func TestPeerStore_Update(t *testing.T) {
	store := New()
	peerV1 := &types.PeerInfo{UserID: "peer1", Multiaddress: "/ip4/127.0.0.1/tcp/4001"}
	store.Add(peerV1)

	peerV2 := &types.PeerInfo{UserID: "peer1", Multiaddress: "/ip4/127.0.0.1/tcp/4002"}
	store.Add(peerV2)
...
func TestPeerStore_Remove(t *testing.T) {
	store := New()
	peer := &types.PeerInfo{UserID: "peer1"}
	store.Add(peer)
...
func TestPeerStore_List(t *testing.T) {
	store := New()
	store.Add(&types.PeerInfo{UserID: "peer1"})
	store.Add(&types.PeerInfo{UserID: "peer2"})
...
func TestPeerStore_Concurrency(t *testing.T) {
	...
	for i := 0; i < peerCount; i++ {
		...
		go func(i int) {
			...
			peer := &types.PeerInfo{UserID: peerID}
			store.Add(peer)
		}(i)
	}
